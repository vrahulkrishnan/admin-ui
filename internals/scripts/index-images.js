...
const build = async (inputDir, outputDir) => {
  ...
  const inputOutputPaths = imagePaths.map(p => ({ name: getImageImportNameFromFilename(p), path: getImagePathNameFromFilename(p)}));
  ...
  const contents = `
    ...
    export type imageMapKeys = ${Object.keys(inputOutputMap).map(k => `'${k}'`).join(' | ')};
  `;
  fs.outputFileSync(path.join(outputDir, 'index.ts'), contents);
};

const OUTPUT_DIR = 'app/.images';
const INPUT_DIR = 'app/assets/images';
